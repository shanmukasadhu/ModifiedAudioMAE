...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler,name_of_exp):
    output_dir = Path(args.output_dir)
    epoch_name = str(epoch)
    if loss_scaler is not None:
        checkpoint_paths = [output_dir / (f'{name_of_exp}_checkpoint-%s.pth' % epoch_name)]
        ...
        model.save_checkpoint(save_dir=args.output_dir, tag=f"{name_of_exp}_checkpoint-%s" % epoch_name, client_state=client_state)
